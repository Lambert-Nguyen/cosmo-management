...
def test_idempotent_task_creation():
    ...
    # Assertions
    if count1 == 1 and count2 == 0 and tasks_after_first == tasks_after_second == 1:
        print("🎉 IDEMPOTENCE TEST PASSED: Second call created no duplicates!")
    else:
        print(f"❌ IDEMPOTENCE TEST FAILED: Expected (1, 0, 1, 1), got ({count1}, {count2}, {tasks_after_first}, {tasks_after_second})")
...
def test_constraint_integrity():
    ...
    try:
        ...
        if "uniq_template_task_per_booking" in str(e):
